...
class ForgetTest(unittest.TestCase):
    ...
    def test_dry_run(self, mock_execute):
        ...
        mock_execute.assert_called_with(["restic", "--json", "forget", "--dry-run"])
    ...
    def test_forget_and_prune(self, mock_execute):
        ...
        mock_execute.assert_called_with(["restic", "--json", "forget", "--prune"])
                    ...
                    "keep": [
                        {
                            ...
                            "tree": "4483c2c6c1386abb9f47497cf108bab19e09c42430d32cd640a4f6f97137841f",  # pylint: disable-next=line-too-long
                            "paths": ["/tmp/tmp6ew1vzp2/mydata.txt"],
                            "hostname": "ecb5551395ae",
                            "username": "circleci",
                            "uid": 3434,
                            "gid": 3434,
                            "id": "3f6de49c6461ffd42900a204655708a3e136a3814abe298c07f27e412e2b6a43",  # pylint: disable-next=line-too-long
                            "short_id": "3f6de49c",
                        }
                    ],
                    "remove": None,
...
""".lstrip()
    ...
    @mock.patch.object(forget.command_executor, "execute")
    def test_forget_raises_exception_when_response_is_invalid_json(self, mock_execute):
        mock_execute.return_value = "{{{{{{{[[[[[["
